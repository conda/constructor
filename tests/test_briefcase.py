...
def test_render_templates_with_write_condarc():
    """Test that .condarc is written when write_condarc is True with channel settings."""
    info = mock_info.copy()
    info["write_condarc"] = True
    info["channels"] = ["conda-forge", "defaults"]
    payload = Payload(info)
    rendered_templates = payload.render_templates()
    ...
    assert "Writing .condarc" in text
    assert '"%BASE_PATH%\\.condarc"' in text
    assert "conda-forge" in text
    assert "defaults" in text


def test_render_templates_with_condarc_dict():
    """Test that .condarc is written when condarc is provided as a dict."""
    info = mock_info.copy()
    info["condarc"] = {
        "channels": ["my-channel"],
        "channel_alias": "https://foo/bar",
    }
    payload = Payload(info)
    rendered_templates = payload.render_templates()
    ...
    assert "https://foo/bar" in text


def test_render_templates_with_condarc_string():
    """Test that .condarc is written when condarc is provided as a string."""
    info = mock_info.copy()
    ...
    assert '"%BASE_PATH%\\.condarc"' in text
    assert "my-channel" in text
    assert "ssl_verify" in text


def test_render_templates_without_condarc():
    """Test that .condarc block is not rendered when no condarc settings are provided."""
    info = mock_info.copy()
    payload = Payload(info)
    rendered_templates = payload.render_templates()
    ...
    assert "Writing .condarc" not in text
