...
def make_nsi(info, dir_path, extra_files=None, temp_extra_files=None):
    ...
    for key, value in [
        ...
        ('@CUSTOM_WELCOME_FILE@', custom_nsi_insert_from_file(info.get('welcome_file', '')) if ppd['custom_welcome'] else ''),
        ('@CUSTOM_CONCLUSION_FILE@', custom_nsi_insert_from_file(info.get('conclusion_file', '')) if ppd['custom_conclusion'] else ''),
