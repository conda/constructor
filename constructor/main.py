...
def main_build(dir_path, output_dir='.', platform=cc_platform,
               ...
               dry_run=False, frozen_dir=None, envdir=None):
    ...
    info = construct_parse(construct_path, platform, frozen_dir)
    ...
    with ExitStack() as stack:
        if not envdir:
            envdir = stack.enter_context(TemporaryDirectory())

        fcp_main(info, envdir=envdir, verbose=verbose, dry_run=dry_run)
        if dry_run:
            print("Dry run, no installer created.")
            return

        print("Creating env archive at {}".format(os.path.join(output_dir, "installer.conda")))
        pkg_api.create(info['_env_path'], None, 'installer.conda', out_folder=output_dir)
...
def main():
    ...
    p.add_argument("DIRECTORY", help="directory where your desired construct.yaml lives")
    ...
    p.add_argument('-e', '--envdir', help="base folder to create envs in.  Defaults to a tempdir.")
    ...
    main_build(dir_path, output_dir=args.output_dir, platform=args.platform,
               ...
               dry_run=args.dry_run, envdir=abspath(expanduser(args.envdir)))
