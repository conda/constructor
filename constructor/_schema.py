...
class ConstructorConfiguration(BaseModel):
    ...
    """
    docker_image: Literal["tar"] | None = None
    """
    If set, builds a docker image using the Dockerfile generated by constructor and saves it as a portable tarball.
    As of now, the only supported image format is `tar`. Additional formats like `gz` or `zst` may be added in the future.
    This option has no effect if `docker_base_image` is not set.
